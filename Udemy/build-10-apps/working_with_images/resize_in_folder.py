    ...
    for img in os.listdir(path):
        if img.endswith(".png") or img.endswith(".jpg"):
            ...
            if detect_orientation(image) == "horizontal":
                resized_image = cv2.resize(image,(int(longer(x,y)),int(shorter(x,y))), interpolation=cv2.INTER_CUBIC)
            elif detect_orientation(image) == "vertical":
                resized_image = cv2.resize(image,(int(shorter(x,y)),int(longer(x,y))), interpolation=cv2.INTER_CUBIC)
            else:
                resized_image = cv2.resize(image,(int(longer(x,y)),int(longer(x,y))), interpolation=cv2.INTER_CUBIC)
